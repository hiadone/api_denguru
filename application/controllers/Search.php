...
class Search extends CB_Controller
{
	...
	protected function _index($config)
	{
		...
		if($scategory && is_array($scategory)){
        	...
        	$set_join[] = array("
				(select cit_id,brd_id,cit_order,cit_name,cit_file_1,cit_review_average,cit_price,cit_price_sale,cbr_id,cit_version from cb_cmall_item ".$cmallwhere.") as cb_cmall_item",'cmall_item.brd_id = board.brd_id','inner');
		  ...
		  //       }
		        ...
		        if($skind && is_array($skind)){
	        	 ...
	        	 //    $this->db->join('cmall_kind_rel','cmall_item.cit_id = cmall_kind_rel.cit_id','inner');
	        	    ...
	        	    $this->db->join('(select cit_id,ckd_id from cb_cmall_kind_rel where ckd_id in ('.implode(",",$skind).') ) AS cb_cmall_kind_rel','cmall_item.cit_id = cmall_kind_rel.cit_id','inner');
