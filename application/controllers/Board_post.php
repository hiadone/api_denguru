...
class Board_post extends CB_Controller
{
	 ...
	 * 게시물 열람 페이지입니다
	 */
	public function post_get($post_id = 0, $print = false)
	{
		// 이벤트 라이브러리를 로딩합니다
		...
		if ($print === false) {
			...
			$view['layout'] = $this->managelayout->front($layoutconfig, $this->cbconfig->get_device_view_type());
			// $this->data = $view;
			$this->data = array('post'=> $view['view']['post'], 'board'=> $view['view']['board'], 'list'=>$view['view']['list']);
	...
	public function crawl_get($cit_id = 0, $print = false)
	{
		...
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_board_post_crawl';
		$this->load->event($eventname);
		...
		$post = $this->Post_model->get_one(element('post_id', $crawl));
		$view['view']['post'] = $post;
		...
		unset($view['view']['board']['brd_content']);
