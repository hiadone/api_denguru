...
class Denguruapi extends CI_Controller
{
    ...
    public function get_popular_brd_attr($brd_id = 0, $limit = 10)
    {
        ...
        if ( ! $data = $this->CI->cache->get($cachename)) {
            ...
            foreach($result as $val){
                ...
                if(element('cat_id',$val) == '4' || element('cat_id',$val) == '5' || element('cat_id',$val) == '6'){
                    $flag= false;
                    break;
                }
            }
            ...
            if($flag){
                 array_unshift($result,array('cnt' => 9999, 'cat_value' => '전견종' , 'cat_id' => '2'));
            }
    ...
    public function get_all_crawl($brd_id = 0)
    {
        ...
        $array = array(
            'brd_order_url', 'brd_register_url', 'brd_orderstatus_url'
        );
        foreach ($array as $value) {
            $url=array();
            if(!empty(element($value,$result))){

                $url = parse_url(element($value,$result));    
                $result[$value] = str_replace(array('m.','www.'),"",$url['host']).$url['path'];
            }
        }
        return $result;
    }
        ...
        if (element('cre_file', $review)) {
            ...
            if ($file && is_array($file)) {
                foreach ($file as $fkey => $fvalue) {
                    ...
                    // if (file_exists(BASEPATH . config_item('uploads_dir').'/cmall_review'.element('rfi_filename', $fvalue).'01.png')) 
                    ...
                    if (element('cre_file', $value)) {
                        ...
                        foreach($file as $fval){
                            if (element('rfi_filename', $fval)) {
                                ...
                                // if (file_exists(BASEPATH . config_item('uploads_dir').'/cmall_review'.element('rfi_filename', $fval).'01.png')) 
