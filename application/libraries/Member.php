	...
	public function get_member()
	{
		if ($this->is_member()) {
			if (empty($this->mb)) {
				...
				$pet = $this->CI->Member_pet_model->get('','',array('mem_id' => element('mem_id', $member)),'1','','pet_main','desc');
				
				if ($pet && is_array(element(0,$pet))) {
					$member = array_merge($member, element(0,$pet));
				}
