...
class Restful extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        $this->_enable_xss = ($this->CI->config->item('global_xss_filtering') === TRUE);
        ...
        $this->CI->output->parse_exec_vars = FALSE;
        ...
        $supported_formats = $this->CI->config->item('rest_supported_formats');
        ...
        $this->CI->lang->load('rest_controller', $language, FALSE, TRUE, __DIR__."/../");
        ...
        if ($this->CI->config->item('rest_ip_blacklist_enabled') === TRUE)
        {
        ...
        $check_cors = $this->CI->config->item('check_cors');
        if ($check_cors === TRUE)
        {
            $this->_check_cors();
        }
        ...
        {
            $this->request->body = $this->CI->format->factory($this->request->body, $this->request->format)->to_array();
        ...
        if ($this->CI->config->item('rest_database_group') && ($this->CI->config->item('rest_enable_keys') || $this->CI->config->item('rest_enable_logging')))
        {
        ...
        if ($this->CI->config->item('rest_enable_keys') && $this->auth_override !== TRUE)
        {
            $this->_allow = $this->_detect_api_key();
        }

        // Only allow ajax requests
        if ($this->CI->input->is_ajax_request() === FALSE && $this->CI->config->item('rest_ajax_only'))
        {
            // Display an error response
            $this->response([
                    $this->CI->config->item('rest_status_field_name') => FALSE,
                    $this->CI->config->item('rest_message_field_name') => $this->CI->lang->line('text_rest_ajax_only')
                ], self::HTTP_NOT_ACCEPTABLE);
        }

        // When there is no specific override for the current class/method, use the default auth value set in the config
        if ($this->auth_override === FALSE &&
            (! ($this->CI->config->item('rest_enable_keys') && $this->_allow === TRUE) ||
            ($this->CI->config->item('allow_auth_and_keys') === TRUE && $this->_allow === TRUE)))
        {
            $rest_auth = strtolower($this->CI->config->item('rest_auth'));
            switch ($rest_auth)
            {
                case 'basic':
            ...
            if ($this->CI->config->item('rest_ip_whitelist_enabled') === TRUE)
            {
    ...
    {   
        ...
        if ($this->CI->config->item('rest_enable_logging') === TRUE)
        {
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this->CI->config->item('force_https') && $this->request->ssl === FALSE)
        {
            $this->response([
                    $this->CI->config->item('rest_status_field_name') => FALSE,
                    $this->CI->config->item('rest_message_field_name') => $this->CI->lang->line('text_rest_unsupported')
                ], self::HTTP_FORBIDDEN);
        }
        ...
        if ($this->CI->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE)
        {
            if ($this->CI->config->item('rest_enable_logging') && $log_method)
            {
                $this->_log_request();
            }
            ...
            $this->response([
                    $this->CI->config->item('rest_status_field_name') => FALSE,
                    $this->CI->config->item('rest_message_field_name') => sprintf($this->CI->lang->line('text_rest_invalid_api_key'), $this->rest->key)
                ], self::HTTP_FORBIDDEN);
        }

        // Check to see if this key has access to the requested controller
        if ($this->CI->config->item('rest_enable_keys') && $use_key && empty($this->rest->key) === FALSE && $this->_check_access() === FALSE)
        {
            if ($this->CI->config->item('rest_enable_logging') && $log_method)
            {
                $this->_log_request();
            ...
            $this->response([
                    $this->CI->config->item('rest_status_field_name') => FALSE,
                    $this->CI->config->item('rest_message_field_name') => $this->CI->lang->line('text_rest_api_key_unauthorized')
        ...
        if (! method_exists($this, $controller_method))
        {
            $this->response([
                    $this->CI->config->item('rest_status_field_name') => FALSE,
                    $this->CI->config->item('rest_message_field_name') => $this->CI->lang->line('text_rest_unknown_method')
                ], self::HTTP_METHOD_NOT_ALLOWED);
        }

        // Doing key related stuff? Can only do it if they have a key right?
        if ($this->CI->config->item('rest_enable_keys') && empty($this->rest->key) === FALSE)
        {
            // Check the limit
            if ($this->CI->config->item('rest_enable_limits') && $this->_check_limit($controller_method) === FALSE)
            {
                $response = [$this->CI->config->item('rest_status_field_name') => FALSE, $this->CI->config->item('rest_message_field_name') => $this->CI->lang->line('text_rest_api_key_time_limit')];
            ...
            if($authorized === FALSE)
            {
                ...
                $response = [$this->CI->config->item('rest_status_field_name') => FALSE, $this->CI->config->item('rest_message_field_name') => $this->CI->lang->line('text_rest_api_key_permissions')];
                $this->response($response, self::HTTP_UNAUTHORIZED);
            }
        }

        //check request limit by ip without login
        elseif ($this->CI->config->item('rest_limits_method') == "IP_ADDRESS" && $this->CI->config->item('rest_enable_limits') && $this->_check_limit($controller_method) === FALSE)
        {
            $response = [$this->CI->config->item('rest_status_field_name') => FALSE, $this->CI->config->item('rest_message_field_name') => $this->CI->lang->line('text_rest_ip_address_time_limit')];
            $this->response($response, self::HTTP_UNAUTHORIZED);
        }

        // No key stuff, but record that stuff is happening
        elseif ($this->CI->config->item('rest_enable_logging') && $log_method)
        {
        ...
        {
            if ($this->CI->config->item('rest_handle_exceptions') === FALSE) {
                throw $ex;
            }
    ...
    public function response($data = NULL, $http_code = NULL)
    {
        ...
        elseif ($data !== NULL)
        {
            ...
            if (method_exists($this->CI->format, 'to_' . $this->response->format))
            {
                // Set the format header
                $this->CI->output->set_content_type($this->_supported_formats[$this->response->format], strtolower($this->CI->config->item('charset')));
                $output = $this->CI->format->factory($data)->{'to_' . $this->response->format}();

                // An array must be parsed as a string, so as not to cause an array to string error
                // Json is the most appropriate form for such a data type
                if ($this->response->format === 'array')
                {
                    $output = $this->CI->format->factory($output)->{'to_json'}();
                }
            }
            else
            {
                ...
                {
                    $data = $this->CI->format->factory($data)->{'to_json'}();
                }
    ...
    {
        $default_format = (string) $this->CI->config->item('rest_default_format');
    ...
    protected function _detect_output_format()
    {
        ...
        if ($this->CI->config->item('rest_ignore_http_accept') === FALSE && $http_accept !== NULL)
        {
    ...
    protected function _detect_api_key()
    {
        ...
        $api_key_variable = $this->CI->config->item('rest_key_name');
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->CI->input->server($key_name)))
        {
            if ( ! ($row = $this->rest->db->where($this->CI->config->item('rest_key_column'), $key)->get($this->CI->config->item('rest_keys_table'))->row()))
            {
                return FALSE;
            }

            $this->rest->key = $row->{$this->CI->config->item('rest_key_column')};
            ...
            if (empty($row->is_private_key) === FALSE)
            {
                ...
                if (isset($row->ip_addresses))
                {
                    ...
                    {
                        if ($this->CI->input->ip_address() === trim($ip_address))
                        {
    ...
    {
        $lang = $this->CI->input->server('HTTP_ACCEPT_LANGUAGE');
        if ($lang === NULL)
        {
            return NULL;
        }
    ...
    protected function _log_request($authorized = FALSE)
    {
        ...
        $is_inserted = $this->rest->db
            ->insert(
                $this->CI->config->item('rest_logs_table'), [
                'uri' => $this->CI->uri->uri_string(),
                'method' => $this->request->method,
                'params' => $this->_args ? ($this->CI->config->item('rest_logs_json_params') === TRUE ? json_encode($this->_args) : serialize($this->_args)) : NULL,
    ...
    protected function _check_limit($controller_method)
    {
        ...
        switch ($this->CI->config->item('rest_limits_method'))
        {
          case 'IP_ADDRESS':
            $limited_uri = 'ip-address:' .$this->CI->input->ip_address();
            $api_key = $this->CI->input->ip_address();
            break;
          ...
          case 'ROUTED_URL':
          default:
            $limited_uri = $this->CI->uri->ruri_string();
        ...
        $result = $this->rest->db
            ...
            ->get($this->CI->config->item('rest_limits_table'))
            ->row();
        ...
        if ($result === NULL)
        {
            ...
            $this->rest->db->insert($this->CI->config->item('rest_limits_table'), [
                'uri' => $limited_uri,
                'api_key' =>$api_key,
                'count' => 1,
                'hour_started' => time()
        ...
        elseif ($result->hour_started < (time() - $time_limit))
        {
            ...
            $this->rest->db
                ...
                ->set('count', 1)
                ->update($this->CI->config->item('rest_limits_table'));
        }
    ...
    protected function _auth_override_check()
    {
        ...
        $auth_override_class_method = $this->CI->config->item('auth_override_class_method');
        ...
        $auth_override_class_method_http = $this->CI->config->item('auth_override_class_method_http');
    ...
    protected function _perform_ldap_auth($username = '', $password = NULL)
    {
        ...
        $this->CI->config->load('ldap.php', TRUE);

        $ldap = [
            'timeout' => $this->CI->config->item('timeout', 'ldap'),
            'host' => $this->CI->config->item('server', 'ldap'),
            'port' => $this->CI->config->item('port', 'ldap'),
            'rdn' => $this->CI->config->item('binduser', 'ldap'),
            'pass' => $this->CI->config->item('bindpw', 'ldap'),
            'basedn' => $this->CI->config->item('basedn', 'ldap'),
        ];
    ...
    protected function _perform_library_auth($username = '', $password = NULL)
    {
        ...
        $auth_library_class = strtolower($this->CI->config->item('auth_library_class'));
        $auth_library_function = strtolower($this->CI->config->item('auth_library_function'));
        ...
        {
            $this->CI->load->library($auth_library_class);
        }
    ...
    protected function _check_login($username = NULL, $password = FALSE)
    {
        ...
        $auth_source = strtolower($this->CI->config->item('auth_source'));
        $rest_auth = strtolower($this->CI->config->item('rest_auth'));
        $valid_logins = $this->CI->config->item('rest_valid_logins');

        if ( ! $this->CI->config->item('auth_source') && $rest_auth === 'digest')
        {
            // For digest we do not have a password passed as argument
            return md5($username.':'.$this->CI->config->item('rest_realm').':'.(isset($valid_logins[$username]) ? $valid_logins[$username] : ''));
        }
    ...
    protected function _check_php_session()
    {
        ...
        $key = $this->CI->config->item('auth_source');
        ...
        if ( ! $this->session->userdata($key))
        {
            ...
            $this->response([
                    $this->CI->config->item('rest_status_field_name') => FALSE,
                    $this->CI->config->item('rest_message_field_name') => $this->CI->lang->line('text_rest_unauthorized')
                ], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _prepare_basic_auth()
    {
        ...
        if ($this->CI->config->item('rest_ip_whitelist_enabled'))
        {
            $this->_check_whitelist_auth();
        }
        ...
        $http_auth = $this->CI->input->server('HTTP_AUTHENTICATION');
    ...
    protected function _prepare_digest_auth()
    {
        ...
        if (strcasecmp($digest['response'], $valid_response) !== 0)
        {
            ...
            $this->response([
                    $this->CI->config->item('rest_status_field_name') => FALSE,
                    $this->CI->config->item('rest_message_field_name') => $this->CI->lang->line('text_rest_invalid_credentials')
                ], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _check_blacklist_auth()
    {
        ...
        if (preg_match($pattern, $this->CI->config->item('rest_ip_blacklist')))
        {
            // Display an error response
            $this->response([
                    $this->CI->config->item('rest_status_field_name') => FALSE,
                    $this->CI->config->item('rest_message_field_name') => $this->CI->lang->line('text_rest_ip_denied')
                ], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    {
        $whitelist = explode(',', $this->CI->config->item('rest_ip_whitelist'));
        ...
        if (in_array($this->CI->input->ip_address(), $whitelist) === FALSE)
        {
            $this->response([
                    $this->CI->config->item('rest_status_field_name') => FALSE,
                    $this->CI->config->item('rest_message_field_name') => $this->CI->lang->line('text_rest_ip_unauthorized')
                ], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    {
        $rest_auth = $this->CI->config->item('rest_auth');
        $rest_realm = $this->CI->config->item('rest_realm');
    ...
    protected function _log_access_time()
    {
        ...
        return $this->rest->db->update(
                $this->CI->config->item('rest_logs_table'), $payload, [
                'id' => $this->_insert_id
            ]);
    }
    ...
    protected function _check_access()
    {
        ...
        if ($this->CI->config->item('rest_enable_access') === FALSE)
        {
            return TRUE;
        }
        ...
        $accessRow = $this->rest->db
            ...
            ->get($this->CI->config->item('rest_access_table'))->row_array();
        ...
        return $this->rest->db
            ...
            ->get($this->CI->config->item('rest_access_table'))
            ->num_rows() > 0;
    }
    ...
    protected function _check_cors()
    {
        ...
        $allowed_headers = implode(' ,', $this->CI->config->item('allowed_cors_headers'));
        $allowed_methods = implode(' ,', $this->CI->config->item('allowed_cors_methods'));

        // If we want to allow any domain to access the API
        if ($this->CI->config->item('allow_any_cors_domain') === TRUE)
        {
        ...
        else
        {
            ...
            if (in_array($origin, $this->CI->config->item('allowed_cors_origins')))
            {
